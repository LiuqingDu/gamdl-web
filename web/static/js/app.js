 ...
 * 重置所有任务并覆盖已有文件
 */
async function resetAllTasksWithOverwrite() {
    if (!confirm('确定要覆盖重新下载所有任务吗？这可能需要很长时间！')) {
        return;
    }

    if (!confirm('这将把所有非运行中的任务重置为等待状态，并且会覆盖现有的文件，确定继续？')) {
        return;
    }

    try {
        const response = await fetch('/api/tasks/reset-all-overwrite', { method: 'POST' });
        const data = await response.json();

        if (!response.ok) {
            alert('覆盖重置失败: ' + (data.detail || data.error));
            return;
        }
...
const resetAllOverwriteBtn = document.getElementById('reset-all-overwrite-btn');
if (resetAllOverwriteBtn) {
    resetAllOverwriteBtn.addEventListener('click', resetAllTasksWithOverwrite);
}
