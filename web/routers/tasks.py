...
@router.post("/reset-all-overwrite", response_model=MessageResponse)
def reset_all_tasks_with_overwrite(session: Session = Depends(get_db_session)):
    """重置所有非运行中的任务为等待状态（覆盖模式）"""
    tasks = session.exec(
        select(Task).where(Task.status != TaskStatus.DOWNLOADING)
    ).all()
    ...
    logger.info(f"覆盖重置了 {count} 个任务")

    # 触发处理
    task_queue.process_next()

    return MessageResponse(message=f"已重置 {count} 个任务（覆盖模式）")
